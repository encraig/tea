...
$(document).ready(function () {
  var signIn = $('#top-bar').find('ul.clearfix > li')[1].id; //signed in

  if (signIn == 'welcome-member') {
    console.log('signed in'); //get the links

    var links = $('#' + signIn).find('.popup-member ul li a'); //loop

    $.each(links, function (i, val) {
      var url = $(val).attr('href');
      var text = $(val).text(); //if sign out
...
//# sourceMappingURL=icon-account.js.map
